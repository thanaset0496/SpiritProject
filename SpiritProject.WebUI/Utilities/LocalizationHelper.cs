...
namespace SpiritProject.WebUI.Utilities
{
    ...
    {
        private readonly static IList<string> _supportedLocalesList = new List<string> { "en", "th" };
        ...
        public static string GetLocalisedUrl(Uri initialUri, IList<string> controllersNames, IList<string> userLangs)
        {
            ...
            if (!isLocaleDefined && isRequestPathToHandle)
            {
                ...
                if (origUrl.Host.Split('/').Count() > 0)
                {
                    SubDomain += origUrl.Host.Split('/')[1];

                }

                // Finally, create new uri with language loocale
                res = string.Format("{0}://{1}:{2}{3}/{4}{5}", origUrl.Scheme, origUrl.Host, origUrl.Port, SubDomain, langVal.ToLower(), normalisedPathAndQuery);
            }
